...
Suspension_Coil_table <- read.csv(file='Suspension_Coil.csv',check.names=F,stringsAsFactors = F)#read as table
Suspension_Coil_table

?total_summary()
total_summary <- Suspension_Coil_table %>% summarize(Mean = mean(PSI), median = median(PSI), varianc= var(PSI), standard_deviation = sd(PSI), .groups = 'keep')#create summary table with multiple columns
total_summary 
